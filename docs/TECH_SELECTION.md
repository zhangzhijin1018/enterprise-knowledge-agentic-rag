...
如果需要先用中文理解“为什么这些技术放在一起”，建议先阅读：

- `docs/ARCHITECTURE_FLOW_GUIDE_CN.md`

本项目的技术组合不是为了堆组件，而是围绕下面这条主线：

```text
FastAPI 接请求
  -> Supervisor 做宏观调度
  -> LangGraph 做业务专家内部工作流
  -> LLM Gateway 做可替换模型接入
  -> Prompt Registry 管理可审查 Prompt
  -> MCP/A2A 管工具和跨 Agent 能力边界
  -> PostgreSQL / Milvus / Redis 承载状态、知识和异步任务
```
...
默认关闭 ReAct planner 和 slot fallback 的原因是：

1. 简单经营分析问题由确定性 Planner 更稳、更快、更可解释；
2. LLM 只服务复杂 planning 或低置信补槽，不能成为 SQL 执行链路；
3. 企业环境需要先可控，再增强智能。
...
- 确认 `ReactPlanValidator / AnalyticsSlotFallbackValidator` 已拦截非法字段和越界 slots。
...
- `core/prompts/catalog.py`：登记 Prompt 名称、用途、输入变量、输出 Schema、风险等级和版本；
- `core/prompts/templates/analytics/`：管理经营分析 ReAct planner 与 slot fallback 模板；
- Pydantic Structured Output：强制 LLM 输出结构化对象。

Prompt 不散落在业务节点中，原因是 prompt 需要版本化、审查、替换和模型适配。经营分析 ReAct prompt 和 slot fallback prompt 都明确约束：只能做规划或补槽，不能生成 SQL，不能绕过权限、SQL Guard 和数据范围治理。
...
Prompt Registry
  ...
  -> ReactPlanValidator / AnalyticsSlotFallbackValidator
  -> AnalyticsPlanner.build_plan_from_slots
  -> SQL Builder / SQL Guard / SQL Gateway
```

其中 Validator 是模型输出进入业务执行链前的硬校验点。Prompt 是软约束，Validator 才是真正的工程安全边界。
