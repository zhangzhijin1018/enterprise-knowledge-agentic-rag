...
##### 四（续）、稠密向量 vs 稀疏向量索引选择

**核心结论：HNSW 更适合稠密向量，稀疏向量应使用专用倒排索引。**

| 维度 | 稠密向量 (Dense) | 稀疏向量 (Sparse) |
|------|-----------------|------------------|
| **典型维度** | 768 / 1024 / 1536 | 250001 (BGE-M3) |
| **数据特征** | 连续分布，各维度均有值 | 稀疏率 >99%，大多数维度为 0 |
| **推荐索引** | **HNSW** / IVF_FLAT | **SPARSE_INVERTED_INDEX** / SPARSE_WAND |
| **索引原理** | 分层图最近邻搜索 | 倒排索引（只索引非零维度） |
| **适用场景** | 语义相似度搜索 | 关键词 / 术语精确匹配 |

3. - ## 技术原因
   
     1. HNSW 适用于稠密向量：
        - HNSW 通过构建多层图结构，对高维稠密向量进行分层贪心搜索
        - 适合维度固定（如 BGE-M3 的 1024 维）、向量分布稠密的场景
     2. Sparse 向量不适合 HNSW：
        - 稀疏向量维度高达 250001（BGE-M3 sparse 维度），但绝大多数为 0
        - 图索引在这种极高维稀疏数据上效率低下
        - 倒排索引（INVERTED_INDEX）天然适合处理稀疏数据：只索引非零维度
