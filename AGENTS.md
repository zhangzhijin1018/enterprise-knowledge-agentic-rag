...
**Python 环境：`conda activate tmf_project`**
...
**核心原则：优先使用成熟优秀的第三方库，不重复造轮子；代码保持精炼可读，性能与功能兼顾；采用业界主流写法。**

#### 8.7.1 第三方库选用原则

| 优先级 | 场景 | 推荐策略 |
|--------|------|----------|
| 优先 | 已有成熟解决方案 | 直接使用成熟开源库（如 `python-a2a`、`FastMCP`） |
| 其次 | 有明确最佳实践 | 参考 agent_learn 等标杆项目的实现模式 |
| 谨慎 | 需要定制开发 | 仅在无合适库时自研，确保接口可替换 |
| 禁止 | 重复造轮子 | 除非性能/功能差距明显，否则复用现有实现 |
...
```
• 参考标杆项目：优先参考 agent_learn 等成熟实现
• 社区主流模式：采用 GitHub Star 高、文档完善、社区活跃的库的用法
• 语言最佳实践：遵循 PEP 8、Google/PEP 257 等代码风格
• 框架原生用法：使用 FastAPI/LangGraph/FastMCP 等框架推荐写法
```
...
#### 8.12.8 SQL Agent 注释要求

SQL Agent 代码必须详细说明安全控制逻辑。

#### 8.12.9 Tool Calling 注释要求
...
### 24.1 SKILL.md - 代码模式参考

`docs/SKILL.md` 包含详细的代码实现模式，是 AGENTS.md 中规则的代码化呈现。
...
### 24.2 PROJECT_COMPLETE_DEVELOPMENT_GUIDE.md - 完整开发指南

`docs/PROJECT_COMPLETE_DEVELOPMENT_GUIDE.md` 包含完整的开发计划和代码模板。
...
```
1. 阅读 AGENTS.md      → 了解开发规范和约束
2. 阅读 SKILL.md       → 了解代码模式
3. 阅读 PROJECT_xxx.md → 了解完整开发计划
4. 按阶段开发         → 从 RAG 检索链路开始
```
...
| 文档 | 位置 | 用途 |
|------|------|------|
| AGENTS.md | 项目根目录 | 核心开发规范 |
| SKILL.md | docs/ | 代码模式参考 |
| PROJECT_COMPLETE_DEVELOPMENT_GUIDE.md | docs/ | 完整开发指南 |
| PRD.md | docs/ | 产品需求文档 |
| ARCHITECTURE.md | docs/ | 架构设计 |
| AGENT_WORKFLOW.md | docs/ | 工作流设计 |
