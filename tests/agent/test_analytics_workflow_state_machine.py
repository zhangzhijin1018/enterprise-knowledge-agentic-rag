...
def build_user_context(user_id: int = 1981) -> UserContext:
    """构造最小用户上下文。"""
...
def test_analytics_workflow_state_machine_happy_path() -> None:
    """happy path 应能产出 finish 阶段与 finish 结果。"""
    ...
    assert state["run_id"] == "run_state_001"
    assert state["trace_id"] == "tr_state_001"
    assert state["final_response"]["meta"]["status"] == "succeeded"


def test_analytics_workflow_state_machine_clarification_path() -> None:
    """缺槽位时应走 clarification 中断态，而不是 failed。"""
    ...
    state = workflow.run_state(
        ...
        trace_id="tr_state_002",
    )

    assert state["workflow_stage"] == AnalyticsWorkflowStage.ANALYTICS_FINISH
    assert state["workflow_outcome"] == AnalyticsWorkflowOutcome.CLARIFY
    assert state["clarification_needed"] is True
    assert state["final_response"]["meta"]["status"] == "awaiting_user_clarification"
    assert state["run_id"] == "run_state_002"
    assert state["trace_id"] == "tr_state_002"
