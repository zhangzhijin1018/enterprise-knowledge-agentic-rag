...
class DataSourceRegistry:
    ...
    def list_data_sources(self, *, only_enabled: bool = True) -> list[dict]:
        ...
        base_sources = {
            key: definition
            for key, definition in self.schema_registry._build_default_data_sources().items()  # noqa: SLF001
        }
        overrides = {}
        if self.data_source_repository is not None:
            overrides = {item["key"]: item for item in self.data_source_repository.list_data_sources(only_enabled=False)}

        merged_rows: list[dict] = []
        for key, base_definition in base_sources.items():
